...
dftaxes = tas.df_remove_chars(
    dftaxes, rowlist, charlist
)  # created function to modify and replace dftaxes
...
dfwage = tas.df_remove_chars(
    dfwage, ["Annual Average Wage"], charlist
)  # second item needs to be a list
...
state_list = (
    dftaxes["State"].unique().tolist()
)  # this will review the column names state and find all the unique values. then using tolist it will create a list name state_list
...
for state in state_list:
    ...
    stateTax, rate, bracket = tas.statetaxes(
        tax_row, "Single Rate", "Single Bracket", salary_federal
    )
    income = salary_federal - stateTax
    # input found variables into the dfincome data frame
    dfincome = dfincome._append(
        {"State": state, "State Rate": rate, "Bracket": bracket, "Income": income},
        ignore_index=True,
    )
dfincome = dfincome.sort_values("Income")  # sort the values in assending order
dfincome.to_csv(
...
dfwageindex["Income Index I"] = dfwageindex["Income"] / (
    31133
)  # average individual income of the united states
dfwageindex["Income Index H"] = dfwageindex["Income"] / (
    61334
)  # average household incme within the usited states
...
df_State_Options["Monthly Expenses"] = (
    df_State_Options["COLindex"] / 100
) * monthly_expenses
...
with sns.axes_style("whitegrid"):
    plt.bar(
        dfincome["State"], dfincome["Income"]
    )  # Use 'Income' directly as the height values
plt.xlabel("State")
...
df_budget_state_filled[states_with_max_income] = df_budget_state[
    states_with_max_income
].fillna(0)
